<?php
/**
 * The Portfolio functions and definitions
 *
...
if ( ! function_exists( 'the_portfolio_setup' ) ) :
	/**
	...
	function the_portfolio_setup() {
		 ...
		 * If you're building a theme based on The Portfolio, use a find and replace
		 * to change 'the-portfolio' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'the-portfolio', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'the-portfolio' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'the_portfolio_custom_background_args',
				array(
 ...
 */
function the_portfolio_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'the_portfolio_content_width', 640 );
}
add_action( 'after_setup_theme', 'the_portfolio_content_width', 0 );
...
function the_portfolio_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'the-portfolio' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'the-portfolio' ),
...
function the_portfolio_scripts() {
	wp_enqueue_style( 'the-portfolio-style', get_stylesheet_uri(), array(), _S_VERSION );
